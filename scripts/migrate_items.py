        ...
        for item_id_str, items in items_dict.items():
            if len(items) > 1:
                ...
                for del_item in deleted_items:
                    ...
                    c.execute("UPDATE players SET equipped_weapon = ? WHERE id = ? AND equipped_weapon = ?",
                              (kept_item['id'], owner_id, del_id))
                    c.execute("UPDATE players SET equipped_armor = ? WHERE id = ? AND equipped_armor = ?",
                              (kept_item['id'], owner_id, del_id))
                    c.execute("UPDATE players SET equipped_accessory = ? WHERE id = ? AND equipped_accessory = ?",
                              (kept_item['id'], owner_id, del_id))
