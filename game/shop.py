...
def buy_shop_item(username, item_name):
    ...
    with db.lock:
        ...
        try:
            c = conn.cursor()
            c.execute(
                "SELECT id, gold, level, class, class_levels FROM players WHERE username = ?",
                (username.lower(),),
            )
            row = c.fetchone()
            if not row:
...
def sell_items(owner_id, target_item_name=None, target_tier=None):
    ...
    with db.lock:
        ...
        try:
            ...
            c.execute(
                "SELECT equipped_weapon, equipped_armor, equipped_accessory FROM players WHERE id = ?",
                (owner_id,),
            )
            p_row = c.fetchone()
            if not p_row:
            ...
            equipped_ids = [
                p_row['equipped_weapon'],
                p_row['equipped_armor'],
                p_row['equipped_accessory'],
            ]
