...
class InfoCog(commands.Cog):
    ...
    async def cmd_register(self, ctx: commands.Context, *args):
        ...
        if success:
            await ctx.send(f"@{ctx.author.name} Successfully registered as '{character_name}' (Class: {class_name.capitalize()})!")
        else:
    ...
    async def cmd_inventory(self, ctx: commands.Context):
        ...
        for db_item in items:
            ...
            equipped = ""
            if db_item['id'] in [player.get('equipped_weapon'), player.get('equipped_armor'), player.get('equipped_accessory')]:
                equipped = " (Equipped)"
    ...
    async def cmd_equip(self, ctx: commands.Context, *, item_name: str = ""):
        ...
        if slot:
            ...
            if current_lvl < req_lvl:
                await ctx.send(f"@{ctx.author.name} ❌ เลเวลไม่ถึง! ไอเทมนี้ต้องการเลเวล {req_lvl} (เลเวลปัจจุบันของคุณคือ {current_lvl})")
                return
    ...
    async def cmd_sell(self, ctx: commands.Context, *, target: str = ""):
        ...
        target = target.strip()
        if not target:
            await ctx.send(f"@{ctx.author.name} วิธีใช้: !sell <ชื่อไอเทม/ID> เพื่อขายทีละชิ้น หรือ !sell R / !sell SR เพื่อขายเหมา")
            return
    ...
    async def cmd_stats(self, ctx: commands.Context):
        ...
        from game.logic import calculate_player_stats, CLASSES, get_required_exp
        import json
